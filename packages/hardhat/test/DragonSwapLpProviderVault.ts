...
describe("DragonSwapLpProviderVault - ERC4626 core and access control", function () {
  ...
  async function deployFixture() {
    ...
    const poolMock = await (await ethers.getContractFactory("PoolMock")).deploy();
    await poolMock.waitForDeployment();

    const positionManagerMock = await (
      await ethers.getContractFactory("PositionManagerMock")
    ).deploy(token0.getAddress(), token1.getAddress());
    await positionManagerMock.waitForDeployment();

    const routerMock = await (
      await ethers.getContractFactory("RouterMock")
    ).deploy(token0.getAddress(), token1.getAddress());
  ...
  it("initializes metadata", async () => {
    const { vault, token1 } = await deployFixture();
    expect(await vault.name()).to.eq(NAME);
    expect(await vault.symbol()).to.eq(SYMBOL);
    expect(await vault.asset()).to.eq(await token1.getAddress());
  });
  ...
  it("withdraw respects access and returns asset", async () => {
    ...
    await expect(vault.connect(user).withdraw(200_000n, await user.getAddress(), await user.getAddress())).to.emit(
      vault,
      "Withdraw",
    );
  ...
  it("only manager can create/modify/collect, only owner can swap", async () => {
    ...
    await expect(vault.connect(user).swapTokensExactIn(true, 0, 0, 0)).to.be.revertedWithCustomError(
      vault,
